...
AndroidAudioRecorderWithAEC::AndroidAudioRecorderWithAEC(
  ...
  if (audioManager_ != nullptr) {
    ...
    audioRecordConstructor_ =
        env->GetMethodID(audioRecordClass, "<init>", "(IIIII)V");
    audioRecordStartRecording_ =
        env->GetMethodID(audioRecordClass, "startRecording", "()V");
    ...
    audioRecordGetAudioSessionId_ =
        env->GetMethodID(audioRecordClass, "getAudioSessionId", "()I");
    audioRecordGetRecordingState_ =
        env->GetMethodID(audioRecordClass, "getRecordingState", "()I");
    audioRecordGetState_ =
        env->GetMethodID(audioRecordClass, "getState", "()I");

    // Cache AcousticEchoCanceler method IDs
    jclass aecClass =
        env->FindClass("android/media/audiofx/AcousticEchoCanceler");
    acousticEchoCancelerIsAvailable_ =
        env->GetStaticMethodID(aecClass, "isAvailable", "()Z");
    acousticEchoCancelerCreate_ = env->GetStaticMethodID(
        aecClass, "create", "(I)Landroid/media/audiofx/AcousticEchoCanceler;");
    acousticEchoCancelerSetEnabled_ =
        env->GetMethodID(aecClass, "setEnabled", "(Z)I");
    ...
    noiseSuppressorIsAvailable_ =
        env->GetStaticMethodID(nsClass, "isAvailable", "()Z");
    noiseSuppressorCreate_ = env->GetStaticMethodID(
        nsClass, "create", "(I)Landroid/media/audiofx/NoiseSuppressor;");
    noiseSuppressorSetEnabled_ =
        env->GetMethodID(nsClass, "setEnabled", "(Z)I");
    noiseSuppressorRelease_ = env->GetMethodID(nsClass, "release", "()V");

    // Cache AudioManager method IDs
    jclass audioManagerClass = env->FindClass("android/media/AudioManager");
    audioManagerGetProperty_ = env->GetMethodID(
        audioManagerClass,
        "getProperty",
        "(Ljava/lang/String;)Ljava/lang/String;");
    audioManagerGenerateAudioSessionId_ =
        env->GetMethodID(audioManagerClass, "generateAudioSessionId", "()I");

    // Check AEC availability
    aecAvailable_ = env->CallStaticBooleanMethod(
        aecClass, acousticEchoCancelerIsAvailable_);
    ...
    if (result == JNI_OK && vm != nullptr) {
      ...
      if (result == JNI_OK && env != nullptr) {
        ...
        if (state == 1) { // AudioRecord.STATE_INITIALIZED
          jint recordingState =
              env->CallIntMethod(audioRecord_, audioRecordGetRecordingState_);
          if (recordingState == 3) { // AudioRecord.RECORDSTATE_RECORDING
            shouldStop_ = false;
            recordingThread_ = std::thread(
                &AndroidAudioRecorderWithAEC::recordingThread, this);
            isRunning_.store(true);
            LOGI("Audio recording started successfully");
          } else {
            LOGE(
                "Failed to start recording - invalid recording state: %d",
                recordingState);
          }
        } else {
...
void AndroidAudioRecorderWithAEC::setAECEnabled(bool enabled) {
  ...
  if (acousticEchoCanceler_ != nullptr) {
    ...
    if (result == JNI_OK && vm != nullptr) {
      result = vm->GetEnv((void **)&env, JNI_VERSION_1_6);
      if (result == JNI_OK && env != nullptr) {
        jint setResult = env->CallIntMethod(
            acousticEchoCanceler_, acousticEchoCancelerSetEnabled_, enabled);
        if (setResult == 0) { // AudioEffect.SUCCESS
          LOGI("AEC %s", enabled ? "enabled" : "disabled");
        } else {
          LOGE(
              "Failed to %s AEC, result: %d",
  ...
  while (!shouldStop_.load()) {
    ...
    if (audioRecord_ != nullptr) {
      jint bytesRead = env->CallIntMethod(
          audioRecord_, audioRecordRead_, audioBuffer, 0, bufferSize);
...
void AndroidAudioRecorderWithAEC::initializeAudioRecord() {
  ...
  jint audioSessionId =
      env->CallIntMethod(audioManager_, audioManagerGenerateAudioSessionId_);
  ...
  // Create AudioRecord
  jclass audioRecordClass = env->FindClass("android/media/AudioRecord");
  jobject audioRecord = env->NewObject(
      audioRecordClass,
      audioRecordConstructor_,
      AUDIO_SOURCE,
      SAMPLE_RATE,
  ...
  if (audioRecord != nullptr) {
    ...
    if (aecAvailable_) {
      jclass aecClass =
          env->FindClass("android/media/audiofx/AcousticEchoCanceler");
      jobject aec = env->CallStaticObjectMethod(
          aecClass, acousticEchoCancelerCreate_, audioSessionId);
    ...
    jclass nsClass = env->FindClass("android/media/audiofx/NoiseSuppressor");
    jboolean nsAvailable =
        env->CallStaticBooleanMethod(nsClass, noiseSuppressorIsAvailable_);

    if (nsAvailable) {
      jobject ns = env->CallStaticObjectMethod(
          nsClass, noiseSuppressorCreate_, audioSessionId);
...
void AndroidAudioRecorderWithAEC::cleanupAEC() {
  ...
  if (result == JNI_OK && vm != nullptr) {
    ...
    if (result == JNI_OK && env != nullptr) {
      if (acousticEchoCanceler_ != nullptr) {
        env->CallVoidMethod(
            acousticEchoCanceler_, acousticEchoCancelerRelease_);
